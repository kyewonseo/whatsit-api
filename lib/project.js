...
var LabelMap = require('../models/labelMap');
...
function insertLabels(projectId, body) {
  return new Promise((resolve, reject) => {
    var labelMap = new LabelMap({
      items: body.data
    })
    labelMap.save(function (err, result) {
      if (err) {
        console.log('labelMap err =>' + err);
        reject(err);
      }
      var labelMapId = result._id;
      console.log('labelMapId=>' + labelMapId);

      updateProject(projectId, {label_map:labelMapId})
        .then((project) => {
          resolve(project);
        })
    })
...
function getLabelsByLabelMapId(id) {
  return new Promise((resolve, reject) => {
    LabelMap.find(
      {"_id": id},
      function(err, labels) {
...
exports.insertLabels = insertLabels
exports.getLabelsByLabelMapId = getLabelsByLabelMapId
