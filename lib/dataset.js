...
function addDataset(body) {
    return new Promise((resolve, reject) => {
        if (body.projectId == null || body.projectId == 'undefined') {
            ...
            if (body.type == 'video') {
            ...
            } else if (body.type == 'bigquery') {

              if (body.data[0] == null) {
                reject('body.data[] is null');
              }
                insertBigquery(body)
                  .then((bigqueryId) => insertDataset(bigqueryId, body))
                  .then((datasetId) => updateProjectDatasets(body.projectId, datasetId))
                  .then((result) => {
                      resolve(result);
                  })
              ...
              //TODO: another type
              reject('dataset type is Invalid');
            }
        }
    })
}

function insertBigquery(body) {
  return new Promise((resolve, reject) => {
    var bigquery = new Bigquery({
      dataset_name: body.data[0].dataset_name,
      table_name: body.data[0].table_name,
      service_account: body.data[0].service_account
    })
    bigquery.save(function (err, result) {
      if (err) {
        console.log('bigquery err =>' + err);
        reject(err);
      }
      var bigqueryId = result._id;
      console.log('bigqueryId=>' + bigqueryId);
      resolve(bigqueryId);
    })
  })
}
...
function insertDataset(objectId, body) {
    return new Promise((resolve, reject) => {
...
function updateDatasetContents(body) {
  return new Promise((resolve, reject) => {

    var data = body.data[0].images;
    ...
    Image.update(
      ...
      function (err, data) {
        if (err) {
          ...
          console.log("updateDatasetContents =>" + JSON.stringify(data));
          resolve(data);
        }
      })
  })
...
function getDatasetContentsByDatasetId(id, count) {
  return new Promise( (resolve, reject) => {
...
exports.updateDatasetContents = updateDatasetContents;
exports.getDatasetContentsByDatasetId = getDatasetContentsByDatasetId;
